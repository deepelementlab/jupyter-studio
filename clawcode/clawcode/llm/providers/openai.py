...
class OpenAIProvider(BaseProvider):
    ...
    async def stream_response(
        ...
        async for chunk in stream:
            ...
            # Capture the last non-null finish_reason so downstream layers can
            # tell e.g. ``length`` / ``content_filter`` apart from a clean
            # ``stop`` on an otherwise-empty response. Several OpenAI-compatible
            # vendors (notably Z.AI's "coding paas" endpoint when thinking is
            # enabled) report ``length`` on the final chunk after spending all
            # the budget inside reasoning_content with zero ``content`` deltas.
        ...
        # Fallback: when the OpenAI-compatible gateway streamed ALL its output
        # into ``reasoning_content`` and left ``content`` empty, surface the
        # reasoning as the answer so the chat UI does not render an empty
        # bubble. Mirrors the same fallback ``send_messages`` already performs
        # for the non-streaming path. Skip when tool_calls are present (the
        # assistant turn IS the tool-call) or when the reasoning looks like a
        # leaked tool-call JSON echo (we don't want raw JSON as content).
        if (
            not (content or "").strip()
            and not tool_calls
            and (thinking or "").strip()
            and not self._looks_like_tool_echo_chunk(thinking)
        ):
            _logger.info(
                "OpenAI-compat stream: empty content, promoting reasoning_content "
                "to content (model=%s, finish_reason=%s, thinking_len=%d)",
                self._model,
                finish_reason,
                len(thinking),
            )
            # Emit one extra content_delta so the front-end reducer treats this
            # as a normal assistant reply alongside the (collapsed) thinking
            # block, rather than a "model returned only a terminator event"
            # empty turn.
            yield ProviderEvent.content_delta(thinking)
            content = thinking
        elif (
            not (content or "").strip()
            and not tool_calls
            and not (thinking or "").strip()
        ):
            # Hard edge case: the upstream stream finished cleanly but produced
            # ZERO usable bytes (no content, no reasoning, no tool calls). Log
            # this loudly so it doesn't masquerade as a UI bug. Common causes:
            # the model/base_url combination is invalid (e.g. an Anthropic-only
            # endpoint accepting an OpenAI-shaped request), ``max_tokens`` was
            # exhausted entirely inside a (silently dropped) reasoning phase,
            # or the provider rejected the request with a 200 + empty body.
            _logger.warning(
                "OpenAI-compat stream: provider returned zero displayable output "
                "(model=%s, finish_reason=%s, input_tokens=%d, output_tokens=%d). "
                "If this is GLM/DeepSeek with thinking enabled, check that your "
                "base_url accepts OpenAI-shaped chat.completions requests.",
                self._model,
                finish_reason,
                input_tokens,
                output_tokens,
