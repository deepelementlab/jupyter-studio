# Troubleshooting Open Jupyter

Understanding how Open Jupyter works will greatly help in troubleshooting issues. Check out the [How Open Jupyter works section in User Guide](user-guide.md#how-open-jupyter-works) first.

Below are detailed information on topics which commonly come up in questions and issues reported about Open Jupyter. Please read through the topics relavant to the issues you are experiencing. You might be able to resolve the issues on your own.

- [Open Jupyter vs JupyterLab Web Application versions](#open-jupyter-vs-jupyterlab-web-application-versions)
- [Open Jupyter vs JupyterLab Web Application settings](#open-jupyter-vs-jupyterlab-web-application-settings)
...
- [After Open Jupyter update, why there is still the old JupyterLab Web Application version?](#after-open-jupyter-update-why-there-is-still-the-old-jupyterlab-web-application-version)
- [macOS write permission issues](#macOS-write-permission-issues)
- [Windows uninstall issues](#Windows-uninstall-issues)
- [Double clicking .ipynb files not launching Open Jupyter](#double-clicking-ipynb-files-not-launching-open-jupyter)
- [Detecting if an issue is specific to Open Jupyter or not](#detecting-if-an-issue-is-specific-to-open-jupyter-or-not)
...
- [Additional resources on Open Jupyter](#additional-resources-on-open-jupyter)

## Open Jupyter vs JupyterLab Web Application versions

When a new version of Open Jupyter is released, it comes with a bundled Python environment installer. The jupyterlab Python package version in that installer is the same as the desktop app version except for the suffix part. For example Open Jupyter version 3.6.3-1 bundles jupyterlab Python package version 3.6.3. Open Jupyter version 3.6.3-2 also bundles the same version of jupyterlab (3.6.3). This allows releasing new versions of Open Jupyter even if the jupyterlab version stays the same.

You can see the version of Open Jupyter by going to Desktop app menu (Hamburger menu on top right) -> About dialog.
...
Open Jupyter is compatible with a wide range of jupyterlab Python package versions (>=3.0.0). So, any custom Python environment with jupyterlab package version >= 3.0.0 can be used in Open Jupyter.

## Open Jupyter vs JupyterLab Web Application settings

Open Jupyter loads JupyterLab Web Application UI in embedded browser. They have different settings except for theme setting which can be synced from Open Jupyter settings to web app settings. Open Jupyter settings are accessed from the hamburger menu on top right of a session window. Web app settings are accessed from the Settings menu shown in the embedded browser view's top section.

Open Jupyter settings are applicable to all sessions. Since JupyterLab Web Application stores settings per user, they are also applied to all sessions, even if they have different Python environments and jupyterlab versions. However, different Python environments can have different JupyterLab extensions installed.

Open Jupyter by default stores workspace configuration (layout of UI, open tabs) per project.

## Settings locations and resetting

Open Jupyter stores user settings, project settings and application data in different locations as JSON files. You can see [configuration and data files section in User Guide](user-guide.md#Configuration-and-data-files) for the locations in different systems. It is safe to delete these files or keys in these files to reset specific configurations.
...
Open Jupyter saves logs in the following locations. Information about crashes, warnings and additional information might be available in logs. For troubleshooting purposes, it is recommended to set log level to Debug.

- On Windows: `%APPDATA%\open-jupyter\logs\main.log`
- On macOS `~/Library/Logs/open-jupyter/main.log`
- On Linux: `~/.config/open-jupyter/logs/main.log` or `$XDG_CONFIG_HOME/open-jupyter/logs/main.log`
...
As mentioned in [how the desktop application works](user-guide.md#how-the-desktop-application-works) section, Open Jupyter requires jupyterlab (>=3.0.0) Python package in the Python environment selected. Open Jupyter launches a new JupyterLab server instance locally using the jupyterlab Python package. A common reason for server launch errors is missing this Python package.
...
Note that Open Jupyter currently supports venv and conda Python environments. Other environments such as `mamba` are not tested.
...
- On macOS: `/Applications/Open Jupyter.app/Contents/Resources/env_installer`
- On Linux: `/opt/JupyterLab/resources/env_installer`

Bundled Python environment is installed to:

- On Windows: `%APPDATA%\open-jupyter\jlab_server`
- On macOS: `~/Library/open-jupyter/jlab_server`
- On Linux: `~/.config/open-jupyter/jlab_server`

New Python environment install paths:

- On Windows: `%APPDATA%\open-jupyter\envs`
- On macOS: `~/Library/open-jupyter/envs`
- On Linux: `~/.config/open-jupyter/envs`

## After Open Jupyter update, why there is still the old JupyterLab Web Application version?

Updating Open Jupyter application doesn't automatically update the Python environment previously used by the application, for various reasons. However, bundled Python environment installer is updated if the new Open Jupyter has a version upgrade other than suffix (`-n`) change. You can compare the versions of Open Jupyter application and jupyterlab package versions in the Python environment as described in [versions section above](#open-jupyter-vs-jupyterlab-web-application-versions).

Open Jupyter shows a notification badge on the title bar as shown below when there is an update available for the bundled environment installation. Clicking on the session info button will open up the environment selection popup and there will be an orange server button to let you update the bundled environment installation with one click.
...
Another way to update the bundled Python environment installation by using the CLI command below as well. Make sure to stop all running Open Jupyter instances before running the command. It will first remove the existing installation and then reinstall the environment.
...
On macOS, bundled Python environment is installed into a non user data directory (`~/Library/open-jupyter/jlab_server`) due to conda environment path limitations. Make sure that you have write permissions to `~/Library` directory. If you are having issues with bundled Python environment in macOS, check that environment is properly installed in there. If `~/Library/open-jupyter/jlab_server/bin/python` file exists and you can manually launch Python by using this path on a macOS Terminal, then your bundled Python environment installation was successful.

Open Jupyter installers for Windows and Linux create `jlab` CLI command as part of the installation process. However, macOS application creates this command at first launch and after updates. This command creation might sometimes fail if the user doesn't have the right permissions. This command is created as a symlink at `/usr/local/bin/jlab`. The symlink points to `/Applications/Open Jupyter.app/Contents/Resources/app/jlab` script that launches the desktop application. If you are having issues with running `jlab` command on macOS, you can try these steps to setup the CLI:
...
- If you are still having issues make sure `/Applications/Open Jupyter.app/Contents/Resources/app/jlab` is executable. You can run the command below to enable execution.

```bash
chmod 755 "/Applications/Open Jupyter.app/Contents/Resources/app/jlab"
```
...
1. Make sure no Open Jupyter or server instance is running (rebooting Windows should terminate them if any)
2. If you want to uninstall any existing Open Jupyter and/or Server installation, go to Add / Remove Programs and remove all of the related installations.
...
## Double clicking .ipynb files not launching Open Jupyter

Open Jupyter installers automatically associate `.ipynb` files with the application. If you are still having issues with launch by double clicking `.ipynb` files, you can fix it by right clicking the file and changing the default opener. Use the system specific `Open with...` dialog to set Open Jupyter as the default application to open `.ipynb` files with.
...
## Detecting if an issue is specific to Open Jupyter or not

Some of the issues are obviously Open Jupyter issues such as inability to launch the application, persisting settings etc. However, we also get issues reported or features requested in this repo for JupyterLab Web Application or other dependency Python libraries. You can check if a problem is specific to Desktop application by testing the same feature in JupyterLab Web Application. You can launch JupyterLab Web Application manually by following the instructions in [Launching JupyterLab Server manually](#Launching-JupyterLab-Server-manually) section.

Another method to launch the web application within the same Python environment is by using the Terminal in Open Jupyter. Just note that this might not always work if you have a bash profile that activates a Python environment on your system by default. Follow these steps to launch the web application from Open Jupyter.
...
4. In order to check if you are using the same Python environment as Open Jupyter, you can run `which python` (`where.exe python` on Windows) command in the Terminal.
...
Open Jupyter automatically downloads and installs new versions on some platforms. If newer versions are causing compatibility issues or other problems you can reinstall an older version. Follow the steps below to disable auto-updates, and revert to an older version.

1. Close all running Open Jupyter instances, make sure all processes have stopped.
...
3. Install older version of Open Jupyter from the [Releases](https://github.com/open-jupyter/open-jupyter/releases/) page.

## `Delete` environment menu item availability in Manage Python environments dialog

In Manage Python environments dialog all the environments discovered by Open Jupyter are listed. However, only the environments installed by Open Jupyter can be deleted from this dialog and if they are not currently in use by a JupyterLab session. That's why some of the environments will not have the `Delete` environment menu available.
...
## Additional resources on Open Jupyter

This repo contains the most up to date documentation on Open Jupyter. However you can refer to the Jupyter blog posts on Open Jupyter for more context and information on the application. Below are the links to the blog posts.

- [Open Jupyter UI Modes (03/11/2024)](https://blog.jupyter.org/open-jupyter-ui-modes-cec528d5e460)
- [Python environment management using Open Jupyter CLI (03/01/2024)](https://blog.jupyter.org/python-environment-management-using-open-jupyter-cli-e57485c9287c)
- [Python environment management in Open Jupyter (02/20/2024)](https://blog.jupyter.org/python-environment-management-in-open-jupyter-3b119c5811d9)
- [Introducing the new Open Jupyter! (02/09/2023)](https://blog.jupyter.org/introducing-the-new-open-jupyter-bca1982bdb23)
- [Open Jupyter — 2022 recap (12/13/2022)](https://blog.jupyter.org/open-jupyter-2022-recap-28bdf00205c6)
- [Open Jupyter App now available! (09/22/2021)](https://blog.jupyter.org/open-jupyter-app-now-available-b8b661b17e9a)
