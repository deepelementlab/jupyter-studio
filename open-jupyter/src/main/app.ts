...
export class JupyterApplication implements IApplication, IDisposable {
  ...
  showSettingsDialog(activateTab?: SettingsDialog.Tab) {
    ...
    const localJupyterLabDefaultPath = getDefaultLocalJupyterLabPath();
    const configuredLocalJupyterLabPath = (
      userSettings.getValue(SettingType.localJupyterLabPath) || ''
    ).trim();
    const effectiveLocalJupyterLabPath =
      configuredLocalJupyterLabPath || localJupyterLabDefaultPath;
    const dialog = new SettingsDialog(
      {
        ...
        useLocalDevJupyterLab: userSettings.getValue(
          SettingType.useLocalDevJupyterLab
        ),
        localJupyterLabPath: configuredLocalJupyterLabPath,
        localJupyterLabPython: userSettings.getValue(
          SettingType.localJupyterLabPython
        ),
        localJupyterLabDevMode: userSettings.getValue(
          SettingType.localJupyterLabDevMode
        ),
        localJupyterLabDefaultPath,
        localJupyterLabSourceTreeOk: looksLikeJupyterLabSourceTree(
          effectiveLocalJupyterLabPath
        ),
        localJupyterLabDevModeBuilt: localJupyterLabDevModeBuilt(
          effectiveLocalJupyterLabPath
        )
      },
      this._registry
  ...
  private _setupAutoUpdater() {
    ...
    const ghRepo = getGithubRepoSlug();
    if (ghRepo) {
      require('update-electron-app')({ repo: ghRepo });
    }
  }
  ...
  private _registerListeners(): void {
    ...
    this._evm.registerEventHandler(
      ...
      () => {
        shell.openExternal(getGithubReleasesPageUrl());
      }
    );
