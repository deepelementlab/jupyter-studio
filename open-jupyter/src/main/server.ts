...
// Default 120s. Local-dev (`--dev-mode`) needs more headroom because
// JupyterLab scans `dev_mode/static` and warms the source-mode extension
// graph on first launch (observed 40-60s on Windows). Honor an override
// via OPEN_JUPYTER_SERVER_LAUNCH_TIMEOUT_MS (ms) for power users / CI.
function getServerLaunchTimeoutMs(isLocalDev: boolean): number {
  const envOverride = parseInt(
    process.env.OPEN_JUPYTER_SERVER_LAUNCH_TIMEOUT_MS || '',
    10
  );
  if (Number.isFinite(envOverride) && envOverride > 0) {
    return envOverride;
  }
  return isLocalDev ? 180000 : 120000;
}
...
function createLaunchScript(
  ...
  // --- Local-dev JupyterLab: append --dev-mode when applicable ------------
  // The registry sets ``environment.isLocalDev`` when the chosen Python
  // interpreter resolves jupyterlab to a checkout under
  // ``localJupyterLabPath``. To actually surface the in-tree extensions
  // built by ``jlpm run build:dev`` we have to start the server with
  // ``--dev-mode``. If ``dev_mode/static`` is not built yet we skip the flag
  // so the user still gets a working Lab using the editable install's wheel
  // assets (degraded but functional).
  if (
    serverInfo.environment.isLocalDev &&
    userSettings.getValue(SettingType.localJupyterLabDevMode) === true &&
    serverInfo.environment.localJupyterLabPath &&
    localJupyterLabDevModeBuilt(serverInfo.environment.localJupyterLabPath)
  ) {
    if (!launchArgs.includes('--dev-mode')) {
      launchArgs.push('--dev-mode');
    }
  }
      ...
      async (resolve, reject) => {
        ...
        const execOptions = {
          ...
          env: {
            ...
            // Surface the local JupyterLab source tree to the Jupyter server
            // so it can locate ``dev_mode/static`` and ``schemas``. Harmless
            // when ``isLocalDev`` is false (we just don't set it).
            ...(this._info.environment.isLocalDev &&
            this._info.environment.localJupyterLabPath
              ? { JUPYTERLAB_DIR: this._info.environment.localJupyterLabPath }
              : {}),
            ...serverEnvVars
          }
        };
        ...
        // Tee child stdout/stderr to electron-log so users can see the
        // Jupyter Server boot progress in the terminal that ran `npm start`
        // (or the file log in production). Without this, a slow boot looks
        // like a silent hang in the UI.
        const teeChunk = (chunk: Buffer | string, stream: 'out' | 'err') => {
          ...
          if (stream === 'err') {
            log.warn(`[jupyter-server stderr] ${text}`);
          } else {
            log.info(`[jupyter-server stdout] ${text}`);
          }
        };
        this._nbServer.stdout?.on('data', c => teeChunk(c, 'out'));
        this._nbServer.stderr?.on('data', c => teeChunk(c, 'err'));

        const launchTimeoutMs = getServerLaunchTimeoutMs(
          !!this._info.environment.isLocalDev
        );
        log.info(
          `Waiting for Jupyter Server at ${this._info.url} (timeout ${launchTimeoutMs}ms, isLocalDev=${!!this._info.environment.isLocalDev})`
        );

        Promise.race([
          waitUntilServerIsUp(this._info.url),
          waitForDuration(launchTimeoutMs)
        ]).then((up: boolean) => {
          if (up) {
            started = true;
            try {
            ...
            log.error(
              `Jupyter Server did not respond at ${this._info.url} within ${launchTimeoutMs}ms. ` +
                `Set OPEN_JUPYTER_SERVER_LAUNCH_TIMEOUT_MS to raise the limit, ` +
                `or run \`jlpm run build:dev\` to speed up --dev-mode startup.`
            );
            this._serverStartFailed();
            reject(
              new Error(
                `Failed to launch Jupyter Server within ${
                  launchTimeoutMs / 1000
                }s (timeout). Check the terminal/logs for the Jupyter Server output and consider raising OPEN_JUPYTER_SERVER_LAUNCH_TIMEOUT_MS.`
              )
            );
          }
        });
