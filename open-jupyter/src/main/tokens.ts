...
export interface IPythonEnvironment {
  ...
  /**
   * When true this environment is bound to a local JupyterLab source checkout
   * (``pip install -e <localJupyterLabPath>``). The Jupyter server will be
   * launched with ``--dev-mode`` and ``JUPYTERLAB_DIR`` set to that path so the
   * front-end built under ``dev_mode/static`` is served instead of the wheel
   * assets.
   */
  isLocalDev?: boolean;
  /**
   * When ``isLocalDev`` is true, absolute path of the JupyterLab source tree
   * pointed to by ``pip show jupyterlab`` -> ``Editable project location``.
   */
