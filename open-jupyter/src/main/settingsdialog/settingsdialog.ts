...
export class SettingsDialog {
  constructor(options: SettingsDialog.IOptions, registry: IRegistry) {
    ...
    const {
      ...
      showTOCInZenMode,
      useLocalDevJupyterLab,
      localJupyterLabPath,
      localJupyterLabPython,
      localJupyterLabDevMode,
      localJupyterLabDefaultPath,
      localJupyterLabSourceTreeOk,
      localJupyterLabDevModeBuilt
    } = options;
      ...
      <div id="container">
        <div id="content-area">
          <jp-tabs id="category-tabs" false="" orientation="vertical">
            ...
            <jp-tab id="tab-local-dev">Local Dev</jp-tab>
            ...
            <jp-tab-panel id="tab-panel-local-dev">
              <div class="row setting-section">
                <div class="row">
                  <jp-checkbox id='local-dev-enabled' type='checkbox' <%= useLocalDevJupyterLab ? 'checked' : '' %> onchange='handleLocalDevEnabledChange();'>
                    Prefer local JupyterLab source (<code>open-jupyter/jupyterlab-main/</code>)
                  </jp-checkbox>
                  <div class="info-icon" title="When enabled, the desktop shell tries to launch the bundled local JupyterLab source tree (via 'pip install -e .'). If anything fails it silently falls back to the bundled / discovered Python environment.">${infoIconSrc}</div>
                </div>
                <div class="row" style="margin-left: 24px; font-size: 12px; color: var(--neutral-foreground-hint);">
                  When enabled, the chosen Python interpreter must have <code>jupyterlab</code> installed editable
                  from the local path below. Otherwise the shell falls back to the bundled environment.
                </div>
              </div>

              <div class="row setting-section">
                <div class="row" style="line-height: 30px;">
                  <label>Local JupyterLab path</label>
                </div>
                <div class="row" style="width: 100%; gap: 6px;">
                  <div style="flex-grow: 1;">
                    <jp-text-field type="text" id="local-jupyterlab-path" value="<%= localJupyterLabPath %>" style="width: 100%;" spellcheck="false" placeholder="<%= localJupyterLabDefaultPath %>"></jp-text-field>
                  </div>
                  <jp-button id='browse-local-jupyterlab-path' onclick='handleBrowseLocalJupyterLabPath();'>Browse</jp-button>
                </div>
                <div class="row" id="local-jupyterlab-path-status" style="margin-top: 4px; font-size: 12px;"></div>
              </div>

              <div class="row setting-section">
                <div class="row" style="line-height: 30px;">
                  <label>Python interpreter for local source</label>
                </div>
                <div class="row" style="width: 100%; gap: 6px;">
                  <div style="flex-grow: 1;">
                    <jp-text-field type="text" id="local-jupyterlab-python" value="<%= localJupyterLabPython %>" style="width: 100%;" spellcheck="false" placeholder="(auto-detect: .venv next to source tree)"></jp-text-field>
                  </div>
                  <jp-button id='browse-local-jupyterlab-python' onclick='handleBrowseLocalJupyterLabPython();'>Browse</jp-button>
                </div>
                <div class="row" style="margin-top: 4px; font-size: 12px; color: var(--neutral-foreground-hint);">
                  Leave empty to auto-detect a sibling <code>.venv</code>, or honor the <code>OPEN_JUPYTER_LOCAL_DEV_PYTHON</code> env var.
                </div>
              </div>

              <div class="row setting-section">
                <div class="row">
                  <jp-checkbox id='local-jupyterlab-dev-mode' type='checkbox' <%= localJupyterLabDevMode ? 'checked' : '' %>>
                    Launch with <code>--dev-mode</code> (serve <code>dev_mode/static</code>)
                  </jp-checkbox>
                  <div class="info-icon" title="Required for the in-tree packages built by 'jlpm run build:dev' (e.g. @jupyterlab/ai-coder-extension) to actually show up in the WebView. Auto-skipped when dev_mode/static/index.html is missing.">${infoIconSrc}</div>
                </div>
                <div class="row" id="local-jupyterlab-dev-mode-status" style="margin-top: 4px; font-size: 12px;"></div>
              </div>

              <script>
                const localDevEnabled = document.getElementById('local-dev-enabled');
                const localJupyterLabPathInput = document.getElementById('local-jupyterlab-path');
                const localJupyterLabPythonInput = document.getElementById('local-jupyterlab-python');
                const localJupyterLabDevModeCheckbox = document.getElementById('local-jupyterlab-dev-mode');
                const localJupyterLabPathStatus = document.getElementById('local-jupyterlab-path-status');
                const localJupyterLabDevModeStatus = document.getElementById('local-jupyterlab-dev-mode-status');
                const browsePathBtn = document.getElementById('browse-local-jupyterlab-path');
                const browsePythonBtn = document.getElementById('browse-local-jupyterlab-python');
                const localJupyterLabDefaultPath = <%- JSON.stringify(localJupyterLabDefaultPath) %>;
                let localJupyterLabSourceTreeOk = <%= localJupyterLabSourceTreeOk ? 'true' : 'false' %>;
                let localJupyterLabDevModeBuilt = <%= localJupyterLabDevModeBuilt ? 'true' : 'false' %>;

                function setLocalDevControlsDisabled(disabled) {
                  for (const el of [localJupyterLabPathInput, localJupyterLabPythonInput, localJupyterLabDevModeCheckbox, browsePathBtn, browsePythonBtn]) {
                    if (disabled) {
                ...
                function refreshLocalDevStatus() {
                  if (localJupyterLabSourceTreeOk) {
                    localJupyterLabPathStatus.style.color = 'var(--neutral-foreground-hint)';
                    localJupyterLabPathStatus.textContent = 'Detected JupyterLab source tree (packages/, dev_mode/, pyproject.toml).';
                  } else {
                    localJupyterLabPathStatus.style.color = 'var(--accent-foreground-rest, #c25a00)';
                    localJupyterLabPathStatus.textContent = 'Path does not look like a JupyterLab source tree. The shell will fall back at runtime.';
                  }

                  if (localJupyterLabDevModeBuilt) {
                    localJupyterLabDevModeStatus.style.color = 'var(--neutral-foreground-hint)';
                    localJupyterLabDevModeStatus.textContent = 'dev_mode/static is built. --dev-mode will be used.';
                  } else {
                    localJupyterLabDevModeStatus.style.color = 'var(--accent-foreground-rest, #c25a00)';
                    localJupyterLabDevModeStatus.textContent = 'dev_mode/static/index.html missing. Run "jlpm run build:dev" in the local source tree.';
                  }
                }

                function handleLocalDevEnabledChange() {
                  setLocalDevControlsDisabled(!localDevEnabled.checked);
                }

                async function handleBrowseLocalJupyterLabPath() {
                  const current = localJupyterLabPathInput.value || localJupyterLabDefaultPath;
                  const selected = await window.electronAPI.selectDirectoryPath(current);
                  if (selected) {
                    localJupyterLabPathInput.value = selected;
                  }
                }

                async function handleBrowseLocalJupyterLabPython() {
                  const current = localJupyterLabPythonInput.value || '';
                  const selected = await window.electronAPI.selectFilePath(current);
                  if (selected) {
                    localJupyterLabPythonInput.value = selected;
                  }
                }

                document.addEventListener('DOMContentLoaded', () => {
                  setLocalDevControlsDisabled(!localDevEnabled.checked);
                  refreshLocalDevStatus();
                });
              </script>
            </jp-tab-panel>
      ...
      <script>
        ...
        function handleApply() {
          ...
          window.electronAPI.setSettings({
            ...
            useLocalDevJupyterLab: localDevEnabled.checked,
            localJupyterLabPath: localJupyterLabPathInput.value.trim(),
            localJupyterLabPython: localJupyterLabPythonInput.value.trim(),
            localJupyterLabDevMode: localJupyterLabDevModeCheckbox.checked
          });
...
export namespace SettingsDialog {
  ...
  export interface IOptions {
    ...
    useLocalDevJupyterLab: boolean;
    localJupyterLabPath: string;
    localJupyterLabPython: string;
    localJupyterLabDevMode: boolean;
    localJupyterLabDefaultPath: string;
    localJupyterLabSourceTreeOk: boolean;
    localJupyterLabDevModeBuilt: boolean;
  }
}
