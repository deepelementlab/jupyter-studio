...
export enum SettingType {
  ...
  // --- Local-dev JupyterLab (open-jupyter/jupyterlab-main source) ------------
  // When useLocalDevJupyterLab is enabled the registry will try to bind the
  // default Python environment to a checkout of the JupyterLab source tree
  // installed in editable mode (``pip install -e .``). When the local-dev
  // resolution fails for any reason the shell silently falls back to the
  // existing bundled / conda / discovered selection logic.
  useLocalDevJupyterLab = 'useLocalDevJupyterLab',
  localJupyterLabPath = 'localJupyterLabPath',
  localJupyterLabPython = 'localJupyterLabPython',
  localJupyterLabDevMode = 'localJupyterLabDevMode'
}
...
export class UserSettings {
  constructor(readSettings: boolean = true) {
    this._settings = {
      ...
      Also, JupyterLab theme is stored as user settings in {USER_DATA}/open-jupyter/lab/.
      ...
      */
      ...
      // jupyter-studio: default to ManagedByWebApp so the AI Coder sidebar
      // (and other JupyterLab side panels) stay visible when the user clicks
      // "New notebook..." on the Welcome page. Upstream Open Jupyter used
      // Zen here, which hides all sidebars and makes AI Coder unreachable.
      uiModeForSingleFileOpen: new Setting<UIMode>(UIMode.ManagedByWebApp),
      showTOCInZenMode: new Setting<boolean>(false),

      useLocalDevJupyterLab: new Setting<boolean>(false),
      localJupyterLabPath: new Setting<string>(''),
      localJupyterLabPython: new Setting<string>(''),
      localJupyterLabDevMode: new Setting<boolean>(true)
    };
