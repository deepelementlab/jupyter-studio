...
export class LabView implements IDisposable {
  constructor(options: LabView.IOptions) {
    ...
    if (!sessionConfig.url) {
      throw new Error(
        'Session URL is not defined. The Jupyter server may not have started successfully.'
      );
    }
  ...
  async setUIMode(uiMode: UIMode) {
    // ManagedByWebApp historically returned early ("let web app manage the
    // layout"). But JupyterLab persists workspace state, so if a *previous*
    // session was in Zen and hid the sidebar tab bars / status bar, those
    // stay hidden across restarts -- the AI Coder icon (and every other
    // sidebar widget) becomes invisible. Run the injected helper for ALL
    // modes; the script special-cases ManagedByWebApp to only restore tab
    // bar / status bar visibility without changing labShell.mode.
  ...
  private _registerWebAppFrontEndHandlers() {
    this._view.webContents.on('dom-ready', () => {
      ...
      // Resolve which uiMode to apply on every dom-ready, NOT only for
      // single-file open. JupyterLab persists workspace state (e.g. side
      // tab bar visibility hidden by Zen mode), so a previous Zen session
      // hides the AI Coder sidebar icon across restarts unless we run
      // jlabDesktop_setUIMode at least once with a non-Zen mode to restore
      // visibility.
      const resolvedUiMode = setToSingleFileUIMode
        ? userSettings.getValue(SettingType.uiModeForSingleFileOpen)
        : this._wsSettings.hasValue(SettingType.uiMode)
        ? this._wsSettings.getValue(SettingType.uiMode)
        : UIMode.ManagedByWebApp;
      ...
      this._view.webContents.executeJavaScript(`
        ...
        UIMode.SingleDocument
          ...
          } else if (uiMode === '${UIMode.ManagedByWebApp}') {
            // Don't touch labShell.mode (let the web app decide), but make
            // sure the side tab bars and status bar are visible. Otherwise
            // a previous Zen session can leave them hidden across restarts,
            // hiding the AI Coder sidebar icon (and every other widget).
            if (!currentState.leftTabBarVisible) {
              labShell.toggleSideTabBarVisibility('left');
            }
            if (!currentState.rightTabBarVisible) {
            ...
            if (statusBar && !currentState.statusBarVisible) {
              statusBar.setHidden(false);
            }
          }
        }
        ...
        jlabDesktop_getLab().then((lab) => {
          lab.restored.then(() => {
            jlabDesktop_setUIMode('${resolvedUiMode}', {
