    ...
    envs.forEach(env => {
      ...
      listLines.push(
        `  [${env.name}], Python path: ${env.path}${
          installedByApp ? ', installed by Open Jupyter' : ''
...
export async function handleEnvUpdateRegistryCommand(argv: any) {
  console.log(`Updating Open Jupyter's Python environment registry...`);
