...
export class SessionWindow implements IDisposable {
  constructor(options: SessionWindow.IOptions) {
    ...
    const winIcon = getDesktopWindowIconPath();
  ...
  private async _createServerForSession() {
    ...
    if (pythonPath) {
      const wsEnv = this._registry.getEnvironmentByPath(pythonPath);
      // Honor user-level ``useLocalDevJupyterLab`` even when the workspace
      // pinned a non-local-dev pythonPath (commonly written automatically on
      // first launch). Without this override, the desktop shell silently
      // boots a JupyterLab from a different environment that does NOT
      // contain our editable AI Coder extension, so the AI Coder side bar
      // and cell footer never appear -- looking like the extension failed
      // to load when in fact a completely different lab is running.
      const wantsLocalDev =
        userSettings.getValue(SettingType.useLocalDevJupyterLab) === true;
      const localDevEnv = this._registry.getCurrentPythonEnvironment();
      if (
        wantsLocalDev &&
        localDevEnv?.isLocalDev &&
        (!wsEnv || !wsEnv.isLocalDev)
      ) {
        log.info(
          `[open-jupyter] Workspace pythonPath "${pythonPath}" is not a ` +
            `local-dev environment; ` +
            `useLocalDevJupyterLab=true is set, so falling back to ` +
            `the registry default "${localDevEnv.path}" (isLocalDev=true).`
        );
        serverOptions.environment = localDevEnv;
      } else {
        serverOptions.environment = wsEnv;
      }
    }
    ...
    this._evm.registerEventHandler(
      ...
      async (event, type: 'notebook' | 'blank') => {
        ...
        try {
          ...
          // Reset content state to Welcome FIRST, then overlay the error
          // in the progress view. Doing it in the opposite order lets
          // _updateContentView() add the Welcome BrowserView on top of
          // the progress view, hiding the error message entirely.
          this._contentViewType = ContentViewType.Welcome;
          this._updateContentView();
          this._updateSessionWindowPositionConfig();
          this._sessionConfigChanged.emit();

          // Force the progress view to be on top by removing+re-adding it
          // (Electron stacks BrowserViews in insertion order).
          if (this._progressViewVisible) {
            this._window.removeBrowserView(this._progressView.view.view);
            this._progressViewVisible = false;
          }
