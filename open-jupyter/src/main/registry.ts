...
import {
  ...
  getDefaultLocalJupyterLabPath,
  getEnvironmentPath,
  getUserHomeDir,
  guessLocalDevPython,
  isBaseCondaEnv,
  isCondaEnv,
  isJupyterlabEditableAt,
  isPortInUse,
  localJupyterLabDevModeBuilt,
  looksLikeJupyterLabSourceTree,
...
export class Registry implements IRegistry, IDisposable {
  constructor() {
    ...
    // --- Local-dev JupyterLab attempt (highest priority) -------------------
    // When the user has opted in to "Prefer local JupyterLab source", try to
    // bind the default Python environment to a Python interpreter where
    // ``jupyterlab`` has been installed editable from the configured local
    // source tree. Any failure here silently falls through to the existing
    // bundled / discovered / conda fallback chain below.
    this._tryResolveLocalDevEnvironment();
    ...
    // Skip when the local-dev resolution already populated `_defaultEnv`; we
    // only want to fall back to bundled / user-set environments when the
    // local-dev attempt was disabled or failed.
    if (!this._defaultEnv) {
      try {
  ...
  /**
   * Attempt to set ``_defaultEnv`` to a Python interpreter that has
   * ``jupyterlab`` installed editable from the configured local JupyterLab
   * source tree (``open-jupyter/jupyterlab-main/``).
   *
   * Best effort: on any failure (feature disabled, paths missing, jupyterlab
   * not installed, version too old, ...) we simply return without setting
   * ``_defaultEnv`` so the existing bundled / discovered / conda fallback
   * chain in the constructor takes over.
   */
  private _tryResolveLocalDevEnvironment(): void {
    const useLocalDev =
      userSettings.getValue(SettingType.useLocalDevJupyterLab) === true;
    if (!useLocalDev) {
      return;
    }

    const configuredLocalPath = (
      userSettings.getValue(SettingType.localJupyterLabPath) || ''
    ).trim();
    const localPath =
      configuredLocalPath || getDefaultLocalJupyterLabPath();
    if (!looksLikeJupyterLabSourceTree(localPath)) {
      log.info(
        `[open-jupyter] Local JupyterLab source not found at "${localPath}"; falling back.`
      );
      return;
    }

    const configuredPy = (
      userSettings.getValue(SettingType.localJupyterLabPython) || ''
    ).trim();
    const pyPath = configuredPy || guessLocalDevPython(localPath);
    if (!pyPath || !fs.existsSync(pyPath)) {
      log.info(
        `[open-jupyter] No Python interpreter found for local JupyterLab at "${localPath}"; falling back.`
      );
      return;
    }

    // Trust an explicitly configured ``localJupyterLabPython`` -- the user
    // has told us this is the local-dev interpreter, so we skip the slow
    // ``pip show jupyterlab`` verification which spawns python and easily
    // times out on a cold venv (observed ~10s+ on Windows). Without this
    // shortcut the local-dev path silently falls back to the system conda
    // env, which does NOT have our editable jupyter_studio_ai installed.
    const trustConfigured =
      configuredPy.length > 0 && configuredPy === pyPath;
    if (!trustConfigured && !isJupyterlabEditableAt(pyPath, localPath)) {
      log.info(
        `[open-jupyter] "${pyPath}" does not have an editable jupyterlab pointing at "${localPath}"; falling back. ` +
          `Hint: run \`pip install -e "${localPath}"\` in that environment.`
      );
      return;
    }
    if (trustConfigured) {
      log.info(
        `[open-jupyter] Trusting configured localJupyterLabPython="${pyPath}" without running pip show (skipping editable verification).`
      );
    }

    let env: IPythonEnvironment | undefined;
    try {
      env = this._resolveEnvironmentSync(pyPath);
    } catch (err) {
      log.warn(
        `[open-jupyter] Local-dev environment resolve failed (${pyPath}): ${
          (err && (err as any).type) || err
        }; falling back.`
      );
      return;
    }
    ...
    env.isLocalDev = true;
    env.localJupyterLabPath = localPath;
    this._defaultEnv = env;

    const devModeReady = localJupyterLabDevModeBuilt(localPath);
    if (!devModeReady) {
      log.warn(
        `[open-jupyter] Using local JupyterLab source at "${localPath}" but ` +
          `dev_mode/static/index.html is missing. Run \`jlpm run build:dev\` ` +
          `in that directory to make --dev-mode usable.`
      );
    } else {
      log.info(
        `[open-jupyter] Local JupyterLab source detected: ${localPath} (python=${pyPath}).`
      );
    }
  }
