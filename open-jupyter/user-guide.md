...
Open Jupyter allows you to change the default Python environment used for all new sessions and also set project (working directory) specific Python environments.
...
Open Jupyter creates new JupyterLab sessions by launching a locally running JupyterLab server and connecting to it. It can also connect to an existing JupyterLab server instance that is running locally or remotely. In order to connect to a server, click the `Connect...` button in the Start section of the Welcome Page.
...
Open Jupyter is a self-contained standalone desktop application which bundles a Python environment. The bundled Python environment comes with several popular Python libraries to make the application ready to use in scientific computing and data science workflows. These packages are `numpy`, `scipy`, `pandas`, `ipywidgets` and `matplotlib`. In order to install additional packages into Open Jupyter's Python environment, you need to follow certain steps during and after the installation as described below.

## Linux Instructions

On Linux, Open Jupyter is typically installed under `/opt/open-jupyter` (exact path may depend on the `.deb`/`.rpm` package), and the bundled Python environment is created in `~/.config/open-jupyter/jlab_server`.

## macOS Instructions

On macOS, Open Jupyter is installed as `/Applications/Open Jupyter.app`, and the bundled Python environment is created in `~/Library/open-jupyter/jlab_server`.

## Windows Instructions

On Windows, there are two installers: one for the Open Jupyter application and one for the bundled Python environment when needed. Use the installer’s default locations (commonly a folder such as `C:\\open-jupyter` for per-machine installs); the bundled environment data lives under `%APPDATA%\\open-jupyter\\jlab_server`.

# Installing New Python Packages

Make sure you installed Open Jupyter following the steps outlined above in order to have required permissions to install new Python packages.
...
Open Jupyter stores user settings, project settings and application data in different locations as JSON files. Below are the storage locations and type of data they contain.
...
- `%APPDATA%\open-jupyter` on Windows
- `$XDG_CONFIG_HOME/open-jupyter` or `~/.config/open-jupyter` on Linux
- `~/Library/Application Support/open-jupyter` on macOS

Open Jupyter also uses custom configuration paths for better user experience and to prevent clashes with existing Jupyter installations.
...
Open Jupyter allows a subset of user settings to be overridden by project settings. Below is the list of settings that can be overridden by each project (working directory) with example values.
...
You can transfer settings from previous JupyterLab installations into Open Jupyter by copying them over to the new configuration path `{jlab-desktop-user-data-dir}` by following these steps:

1. Run `jupyter --paths` to determine where your `config` is stored.
2. Find directory called `lab` in one of these paths.
3. If there is a `user-settings` directory in `lab`, you can copy the `lab` directory over to the configuration path `{jlab-desktop-user-data-dir}` of Open Jupyter to keep your old JupyterLab settings.
...
Open Jupyter provides a convenient way to configure the JupyterLab UI layout. It lets you change the layout using `UI Mode` menu to pre-defined options and the selection is persisted per project.
...
Open Jupyter supports light and dark themes for the application interface. In the settings dialog you can choose `Light`, `Dark` or `System` options to set the desired theme as shown below. `System` option detects the theme used by your OS and sets that as the app theme.
...
# Uninstalling Open Jupyter

## Debian, Ubuntu Linux

```bash
sudo apt-get purge open-jupyter # remove application
rm /usr/bin/jlab # remove command symlink

# to remove application cache and bundled Python environment
rm -rf ~/.config/open-jupyter
```

## Red Hat, Fedora, SUSE Linux

```bash
sudo rpm -e open-jupyter # remove application
rm /usr/bin/jlab # remove command symlink
...
Find `Open Jupyter.app` in Finder (in /Applications or ~/Applications) and move to Trash by using `CMD + Delete`. Clean other application generated files using:

```bash
rm /usr/local/bin/jlab # remove command symlink

# to remove application cache and bundled Python environment
rm -rf ~/Library/open-jupyter
# to remove user data
rm -rf ~/Library/Application\ Support/open-jupyter
# to remove logs
rm -rf ~/Library/Logs/open-jupyter
```

## Windows

On Windows, Open Jupyter is installed in two parts, one for the python environment and another for the application itself. Go to `Windows Apps & Features` dialog using `Start Menu` -> `Settings` -> `Apps` and make sure to uninstall the components in the following order:

- First uninstall Open Jupyter python environment (if applicable, for version v4.0.1-1 and older)

<img src="media/uninstall-windows-python-environment.png" alt="Uninstall Python environment" height=200 />

- Then uninstall Open Jupyter application

<img src="media/uninstall-windows-application.png" alt="Uninstall the application" height=200 />

In order to remove application cache, delete `%APPDATA%\open-jupyter` directory.
