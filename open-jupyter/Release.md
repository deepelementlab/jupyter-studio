...
Open Jupyter uses [tbump](https://github.com/dmerejkowsky/tbump) to bump JupyterLab and the application versions. You can install using:
...
Open Jupyter needs to be versioned with the same major, minor and patch versions as the JupyterLab it bundles. For example, if Open Jupyter is based on JupyterLab 3.1.12, a valid Open Jupyter version is 3.1.12-1 to 3.1.12-n. Last number after `-` is used as the `build number`. This version matching is enforced before Open Jupyter installer binaries are published.

JupyterLab version, that Open Jupyter bundles, is determined by `@jupyterlab/metapackage` dependency version in the [yarn.lock](yarn.lock).

If the JupyterLab version is not changing with the new Open Jupyter release then only increment the build number after `-` (for example `3.1.12-2` to `3.1.12-3`). However, if JupyterLab version is changing with the new Open Jupyter release then reset the build number after `-` to 1 (for example `3.1.12-3` to `3.1.13-1`).
...
Note that after updating the bundled JupyterLab version, it is necessary to bump Open Jupyter version using `tbump` as described in the section below. Run `check_version_match` script before committing the changes to ensure version integrity.
...
1. Create a new release on GitHub as `pre-release`. Set the release `tag` to the value of target application version and prefix it with `v` (for example `v3.1.12-1` for Open Jupyter version `3.1.12-1`). Enter release title and release notes. Release needs to stay as `pre-release` for GitHub Actions to be able to attach installers to the release.
